...
{
  local DATA_DIR="${OPENBOT_ROOT_DIR}/data"
...
function determine_bin_prefix() {
  OPENBOT_BIN_PREFIX=$OPENBOT_ROOT_DIR
  if [ -e "${OPENBOT_ROOT_DIR}/bazel-bin" ]; then
    OPENBOT_BIN_PREFIX="${OPENBOT_ROOT_DIR}/bazel-bin"
  fi
  export OPENBOT_BIN_PREFIX
}
...
function decide_task_dir() {
  ...
  if [ -z "${DISK}" ]; then
    echo "Cannot find portable disk. Fallback to openbot data dir."
    DISK="/openbot"
  fi
...
function start_customized_path() {
  ...
  if [ $? -eq 1 ]; then
    eval "nohup cyber_launch start ${OPENBOT_ROOT_DIR}/modules/${MODULE_PATH}/launch/${MODULE}.launch &"
    sleep 0.5
    is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
    if [ $? -eq 0 ]; then
...
function start_prof_customized_path() {
  ...
  echo "Make sure you have built with 'bash openbot.sh build_prof'"
  LOG="${OPENBOT_ROOT_DIR}/data/log/${MODULE}.out"
  is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
  if [ $? -eq 1 ]; then
    PROF_FILE="/tmp/$MODULE.prof"
    rm -rf $PROF_FILE
    BINARY=${OPENBOT_BIN_PREFIX}/modules/${MODULE_PATH}/${MODULE}
    eval "CPUPROFILE=$PROF_FILE $BINARY \
        --flagfile=modules/${MODULE_PATH}/conf/${MODULE}.conf \
        --log_dir=${OPENBOT_ROOT_DIR}/data/log $@ </dev/null >${LOG} 2>&1 &"
    sleep 0.5
    is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
    if [ $? -eq 0 ]; then
...
function start_fe_customized_path() {
  ...
  is_stopped_customized_path "${MODULE_PATH}" "${MODULE}"
  if [ $? -eq 1 ]; then
    eval "cyber_launch start ${OPENBOT_ROOT_DIR}/modules/${MODULE_PATH}/launch/${MODULE}.launch"
  else
    echo "Module ${MODULE} is already running - skipping."
    return 2
  fi
...
function start_gdb_customized_path() {
  ...
  eval "gdb --args ${OPENBOT_BIN_PREFIX}/modules/${MODULE_PATH}/${MODULE} \
      --flagfile=modules/${MODULE_PATH}/conf/${MODULE}.conf \
      --log_dir=${OPENBOT_ROOT_DIR}/data/log $@"
}
...
function stop_customized_path() {
  ...
  cyber_launch stop "${OPENBOT_ROOT_DIR}/modules/${MODULE_PATH}/launch/${MODULE}.launch"
  if [ $? -eq 0 ]; then
...
function help() {
  cat << EOF
Invoke ". scripts/openbot_base.sh" within docker to add the following commands to the environment:
...
if [ $OPENBOT_IN_DOCKER = "true" ]; then
  create_data_dir
  if [ -z $OPENBOT_BASE_SOURCED ]; then
    determine_bin_prefix
    export OPENBOT_BASE_SOURCED=1
  fi
fi
