...
function determine_test_targets() {
  ...
  for component in $@; do
    ...
    if [ "${component}" = "cyber" ]; then
      test_targets="//cyber/..."
    elif [ -d "${OPENBOT_ROOT_DIR}/modules/${component}" ]; then
...
function run_bazel_coverage() {
  ...
  local count="$(($(nproc) / 2))"
  local job_args="--jobs=${count} --local_resources=memory=HOST_RAM*0.7"
...
function main() {
  ...
  success "Done bazel coverage for ${SHORTHAND_TARGETS:-Openbot}. "
