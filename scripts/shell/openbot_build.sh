...
function determine_build_targets() 
{
  ...
  for component in $@; do
    local build_targets
    if [[ -d "${OPENBOT_ROOT_DIR}/modules/${component}" ]]; then
      build_targets="//modules/${component}/..."
    else
      error "Directory <OPENBOT_ROOT_DIR>/modules/${component} not found. Exiting ..."
      exit 1
    fi
...
function run_bazel_build() {
  ...
  local job_args="--jobs=$(nproc) --local_resources=memory=HOST_RAM*0.7"
  bazel build ${CMDLINE_OPTIONS} ${job_args} -- ${formatted_targets}
}

function build_simulator() {
  local SIMULATOR_TOP_DIR="/openbot-simulator"
  if [ -d "${SIMULATOR_TOP_DIR}" ] && [ -e "${SIMULATOR_TOP_DIR}/build.sh" ]; then
    ...
    if bash build.sh build ${opt}; then
      success "Done building Openbot simulator."
    else
      fail "Building Openbot simulator failed."
    fi
    popd > /dev/null
  fi
}
