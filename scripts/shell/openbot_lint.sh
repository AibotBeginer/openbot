...
# shellcheck source=./openbot.bashrc
...
function run_sh_lint() {
  ...
  sh_dirs=$(printf "${OPENBOT_ROOT_DIR}/%s " ${sh_dirs})
  run find ${sh_dirs} -type f \( -name "*.sh" -or -name "*.bashrc" \) -exec \
    shellcheck -x --shell=bash {} +

  for script in ${OPENBOT_ROOT_DIR}/*.sh; do
...
function run_py_lint() {
  ...
  py_dirs=$(printf "${OPENBOT_ROOT_DIR}/%s " ${py_dirs})
