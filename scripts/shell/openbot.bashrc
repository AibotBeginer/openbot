...
OPENBOT_ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
OPENBOT_IN_DOCKER=false

# If inside docker container
if [ -f /.dockerenv ]; then
  OPENBOT_IN_DOCKER=true
  OPENBOT_ROOT_DIR="/openbot"
fi

export OPENBOT_ROOT_DIR="${OPENBOT_ROOT_DIR}"
export OPENBOT_IN_DOCKER="${OPENBOT_IN_DOCKER}"
export OPENBOT_CACHE_DIR="${OPENBOT_ROOT_DIR}/.cache"
export OPENBOT_SYSROOT_DIR="/opt/openbot/sysroot"
...
function run() {
  if [ "${VERBOSE}" = yes ]; then
    ...
    local errfile="${OPENBOT_ROOT_DIR}/.errors.log"
    echo "${@}" >"${errfile}"
    if ! "${@}" >>"${errfile}" 2>&1; then
      local exitcode=$?
      cat "${errfile}" 1>&2
