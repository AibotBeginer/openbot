...
OPENBOT_DOCS_CFG="${OPENBOT_ROOT_DIR}/openbot.doxygen"
OPENBOT_DOCS_DIR="${OPENBOT_ROOT_DIR}/.cache/docs"

function determine_docs_dir() {
  local doxygen_cfg="${OPENBOT_DOCS_CFG}"
  ...
  if [[ "${output_dir}" != /* ]]; then
    output_dir="${OPENBOT_ROOT_DIR}/${output_dir}"
  fi

  OPENBOT_DOCS_DIR="${output_dir}"
}

function generate_docs() {
  local doxygen_cfg="${OPENBOT_DOCS_CFG}"
  local output_dir="${OPENBOT_DOCS_DIR}"
  ...
  pushd "${OPENBOT_ROOT_DIR}" > /dev/null
  run "${doxygen_cmd}" "${doxygen_cfg}" > /dev/null
  popd > /dev/null

  local elapsed="$(time_elapsed_s ${start_time})"
  success "Openbot docs generated. Time taken: ${elapsed}s"
}

function clean_docs() {
  if [ -d "${OPENBOT_DOCS_DIR}" ]; then
    rm -rf "${OPENBOT_DOCS_DIR}"
    success "Done cleanup openbot docs in ${OPENBOT_DOCS_DIR}"
  else
    success "Nothing to do for empty directory '${OPENBOT_DOCS_DIR}'."
  fi
}
...
function _usage() {
  ...
  info "${TAB}generate    Generate Openbot docs"
  #local doclink="http://0.0.0.0:${OPENBOT_DOCS_PORT}"
  #info "${TAB}start       Start local Openbot docs server at ${doclink}"
  #info "${TAB}shutdown    Shutdown local Openbot docs server at ${doclink}"
  exit 1
}
