...
function _clean_bazel_cache() {
  ...
  if [ -d /openbot-simulator ]; then
    pushd /openbot-simulator >/dev/null
    bazel clean ${opt}
    popd >/dev/null
  fi
}
...
function _clean_config() {
  ...
  bash "${TOP_DIR}/scripts/openbot_config.sh" --clean
}
...
function _clean_docs() {
  local docs_sh="${TOP_DIR}/scripts/openbot_docs.sh"
  if [ -f "${docs_sh}" ]; then
    bash "${docs_sh}" clean
  fi
}
