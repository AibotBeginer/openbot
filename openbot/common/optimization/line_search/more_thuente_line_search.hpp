...
#ifndef OPENBOT_COMMON_OPTIMIZATION_LINE_SEARCH_MORE_THUENTE_LINE_SEARCH_HPP_
#define OPENBOT_COMMON_OPTIMIZATION_LINE_SEARCH_MORE_THUENTE_LINE_SEARCH_HPP_
...
///
class MoreThuenteLineSearch : public LineSearch<MoreThuenteLineSearch>
{
public:
  ///
  ...
  OptimizationProblemT & optimization_problem)
{
  ...
  for (auto step_iterations = 0; step_iterations < m_max_iterations; ++step_iterations) {
    ...
    if (helper_functions::approx_eq(interval.a_u, interval.a_l, m_step_min, EPS)) {
  ...
  const FunctionValueT & f_t, const FunctionValueT & f_l, const FunctionValueT & f_u)
{
  ...
  const auto find_cubic_minimizer = [kStepEps](const auto & f_a, const auto & f_b) -> StepT {
      if (helper_functions::approx_eq(f_a.argument, f_b.argument, kStepEps, kStepEps)) {
        return f_a.argument;
      }
  ...
  } else if (helper_functions::abs_lte(std::abs(f_t.derivative), std::abs(f_l.derivative), kValueEps)) {
...
#endif  // OPENBOT_COMMON_OPTIMIZATION_LINE_SEARCH_MORE_THUENTE_LINE_SEARCH_HPP_
