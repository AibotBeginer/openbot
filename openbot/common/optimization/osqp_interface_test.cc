...
#include "openbot/common/optimization/osqp_interface.hpp"
...
TEST(TestOsqpInterface, BasicQp)
{
  ...
  const std::vector<double> l = {1.0, 0.0, 0.0, -INF};
  const std::vector<double> u = {1.0, 0.7, 0.7, INF};
...

  {
    ...
    OSQPInterface osqp(P_ini, A_ini, q_ini, l_ini, u_ini, 1e-6);
    osqp.optimize();
...

  {
    ...
    OSQPInterface osqp(P_csc, A_csc, q, l, u, 1e-6);
    std::tuple<std::vector<double>, std::vector<double>, int, int, int> result = osqp.optimize();
    check_result(result);
  }

  {
    ...
    std::vector<double> l_ini(4, 0.0);
    std::vector<double> u_ini(4, 0.0);
    OSQPInterface osqp(P_ini_csc, A_ini_csc, q_ini, l_ini, u_ini, 1e-6);
    osqp.optimize();
