...
#ifndef OPENBOT_COMMON_INTERPOLATION_SPLINE_INTERPOLATION_POINTS_2D_HPP_
#define OPENBOT_COMMON_INTERPOLATION_SPLINE_INTERPOLATION_POINTS_2D_HPP_

#include "openbot/common/interpolation/spline_interpolation.hpp"
...
public:
  ...
  common::geometry_msgs::Point getSplineInterpolatedPoint(const size_t idx, const double s) const;
  double getSplineInterpolatedYaw(const size_t idx, const double s) const;

  double getAccumulatedLength(const size_t idx) const;

private:
  void calcSplineCoefficientsInner(const std::vector<common::geometry_msgs::Point> & points);
...
#endif  // OPENBOT_COMMON_INTERPOLATION_SPLINE_INTERPOLATION_POINTS_2D_HPP_
