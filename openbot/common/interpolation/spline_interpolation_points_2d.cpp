...
std::array<std::vector<double>, 3> getBaseValues(const std::vector<common::geometry_msgs::Point> & points)
{
  // calculate x, y
...
template std::vector<double> splineYawFromPoints(const std::vector<common::geometry_msgs::Point> & points);

common::geometry_msgs::Point SplineInterpolationPoints2d::getSplineInterpolatedPoint(const size_t idx, const double s) const
{
  ...
  common::geometry_msgs::Point geom_point;
  geom_point.x = x;
  geom_point.y = y;
  return geom_point;
}
...
void SplineInterpolationPoints2d::calcSplineCoefficientsInner(
  const std::vector<common::geometry_msgs::Point> & points)
{
