...
#include "openbot/common/utils/blocking_queue.hpp"
#include "openbot/common/utils/time.hpp"
#include "openbot/drivers/sensor/common/dispatchable.hpp"
...
struct QueueKey 
{
  ...
  bool operator<(const QueueKey& other) const {
    return std::forward_as_tuple(sensor_id) <
           std::forward_as_tuple(other.sensor_id);
  }
};
...
class OrderedMultiQueue {
 ...
 private:
  struct Queue {
    common::utils::BlockingQueue<std::unique_ptr<Data>> queue;
    Callback callback;
    bool finished = false;
  };
