...
#include "openbot/system/navigation/behavior_tree/plugins/action/truncate_path_local_action.hpp"
...
inline BT::NodeStatus TruncatePathLocal::tick()
{
    ...
    //     [&pose, angular_distance_weight](const common::geometry_msgs::PoseStamped & ps) {
    ...
    // common::nav_msgs::Path output_path;
    // output_path.header = path_.header;
    // output_path.poses = std::vector<common::geometry_msgs::PoseStamped>(
...
inline bool TruncatePathLocal::GetRobotPose(
  std::string path_frame_id, common::geometry_msgs::PoseStamped& pose)
{
...
{
    factory.registerNodeType<openbot::system::navigation::behavior_tree::TruncatePathLocal>(
        "TruncatePathLocal");
}