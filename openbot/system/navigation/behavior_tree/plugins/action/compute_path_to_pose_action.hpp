...
#include "openbot/system/navigation/proto/compute_path_to_pose.pb.h"
 ...
 */
class ComputePathToPoseAction : public BtActionNode<openbot::navigation::ComputePathToPose>
{
    using Action = openbot::navigation::ComputePathToPose;
...
public:
    ...
    static BT::PortsList providedPorts()
    {
        ...
        {
            BT::InputPort<common::geometry_msgs::PoseStamped>("goal", "Destination to plan to"),
            BT::InputPort<common::geometry_msgs::PoseStamped>("start", "Start pose of the path if overriding current robot pose"),
            BT::InputPort<std::string>("planner_id", "", "Mapped name to the planner plugin type to use"),
            BT::OutputPort<common::nav_msgs::Path>("path", "Path created by ComputePathToPose node"),
        });
    }
};
