...
#include "openbot/map/grid_map/grid_map_pcl/GridMapPclLoader.hpp"
#include "openbot/map/grid_map/grid_map_pcl/helpers.hpp"
...
void GridMapPclLoader::initializeGridMapGeometryFromInputCloud() {
  ...
  LOG(INFO) << "Grid map dimensions: " << workingGridMap_.getLength()(0) << " x " << workingGridMap_.getLength()(1);
  LOG(INFO) << "Grid map resolution: " << workingGridMap_.getResolution();
  LOG(INFO) << "Grid map num cells: " << workingGridMap_.getSize()(0) << " x " << workingGridMap_.getSize()(1);
  LOG(INFO) << "Initialized map geometry";
}

void GridMapPclLoader::addLayerFromInputCloud(const std::string& layer) {
  LOG(INFO) << "Started adding layer: " << layer;
  ...
  LOG(WARNING) << "OpemMP not found, defaulting to single threaded implementation";
#else
...
#pragma omp parallel for schedule(dynamic, 10)
  ...
  LOG(INFO) << "Finished adding layer: " << layer;
}
...
void GridMapPclLoader::processGridMapCell(const unsigned int linearGridMapIndex, grid_map::Matrix* gridMapData) {
  ...
  if (isTooFewPointsInCell) {
    LOG(WARNING) << "Less than " << params_.get().gridMap_.minCloudPointsPerCell_ << " points in a cell. Skipping.";
    return;
  }
