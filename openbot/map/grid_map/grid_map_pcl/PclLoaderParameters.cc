...
#include "openbot/map/grid_map/grid_map_pcl/PclLoaderParameters.hpp"
...
bool PclLoaderParameters::loadParameters(const std::string& filename) {
  ...
  if (loadingFailed) {
    LOG(ERROR) << "PclLoaderParameters: Reading from file failed";
    return false;
  }
  ...
  try {
    ...
    LOG(ERROR) << "PclLoaderParameters: Loading parameters failed: " << exception.what();
    return false;
  }
