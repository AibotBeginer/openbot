...
_OPENBOT_ROOT_DIR = ''
_OPENBOT_BAZELRC = '.openbot.bazelrc'

_OPENBOT_CURRENT_BAZEL_VERSION = None
_OPENBOT_MIN_BAZEL_VERSION = '2.0.0'
_OPENBOT_INSIDE_DOCKER = True
_OPENBOT_DOCKER_STAGE = "dev"
...
def docker_stage():
    default_openbot_stage = "dev"
    if not inside_docker():
        return default_openbot_stage
    stage_conf = "/etc/openbot.conf"
    if not os.path.exists(stage_conf) or not os.path.isfile(stage_conf):
        return default_openbot_stage

    with open("/etc/openbot.conf") as f:
        for line in f:
            line = line.strip()
...
def write_to_bazelrc(line):
    with open(_OPENBOT_BAZELRC, 'a') as f:
        f.write(line + '\n')
...
def get_python_path(environ_cp, python_bin_path):
    ...
    for path in all_paths:
        if os.path.isdir(path) and not path.startswith(
                os.path.join(_OPENBOT_ROOT_DIR, "bazel-bin")):
            paths.append(path)
    return paths
...
def setup_common_dirs(environ_cp):
    ...
    cache_dir = os.path.join(_OPENBOT_ROOT_DIR, '.cache')
    dist_dir = os.path.join(_OPENBOT_ROOT_DIR, '.cache/distdir')

    # if cyber/setup.bash not sourced
    if 'OPENBOT_CACHE_DIR' in environ_cp:
        cache_dir = environ_cp['OPENBOT_CACHE_DIR']

    if 'OPENBOT_BAZEL_DIST_DIR' in environ_cp:
        dist_dir = environ_cp['OPENBOT_BAZEL_DIST_DIR']
...
def reset_openbot_bazelrc():
    """Reset file that contains customized config settings."""
    open(_OPENBOT_BAZELRC, 'w').close()
...
def main():
    ...
    global _OPENBOT_ROOT_DIR
    global _OPENBOT_BAZELRC
    global _OPENBOT_CURRENT_BAZEL_VERSION
    global _OPENBOT_INSIDE_DOCKER
    global _INTERACTIVE_MODE
    global _OPENBOT_DOCKER_STAGE

    parser = argparse.ArgumentParser()
    parser.add_argument(
        '--output_file',
        type=str,
        default='.openbot.bazelrc',
        help='Path of the bazelrc file to write to (relative to OPENBOT_ROOT_DIR)')
    ...
    _OPENBOT_ROOT_DIR = default_root_dir()
    _OPENBOT_BAZELRC = os.path.join(_OPENBOT_ROOT_DIR, args.output_file)
    _OPENBOT_INSIDE_DOCKER = inside_docker()
    _OPENBOT_DOCKER_STAGE = docker_stage()
    ...
    try:
        current_bazel_version = check_bazel_version(_OPENBOT_MIN_BAZEL_VERSION)
    except subprocess.CalledProcessError as e:
        print('Error checking bazel version: ',
              e.output.decode('UTF-8').strip())
        raise e

    _OPENBOT_CURRENT_BAZEL_VERSION = convert_version_to_int(
        current_bazel_version)

    reset_openbot_bazelrc()
    ...
    if not _OPENBOT_INSIDE_DOCKER and 'LD_LIBRARY_PATH' in environ_cp:
